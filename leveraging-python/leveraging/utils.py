...
def leveraging_approach(df, protected_attrs, label_name, use_protected, use_sample_weight=False, log='log.txt', model='gbm', num_iter=10, rand_seed=42):
    ...
    for seed in tqdm(range(num_iter)):
        ...
        if model == 'gbm':
            ...
            clf_YgX = RandomForestClassifier(random_state=rand_seed)
        else:
        ...
        compiled_scores_list.append(y_test_leveraged.reshape(-1,1))

        ## perform leveraging approach on validation set
        ## compute prior
        y_valid_prob = clf_YgX.predict_proba(X=X_valid)[:, 1]
        female_msk = (s_valid == 0)
        E_X0 = y_valid_prob[female_msk].mean()
        E_X1 = y_valid_prob[~female_msk].mean()
        P_Y0S0 = y_valid_prob[female_msk].mean() * female_msk.mean()
        P_Y0S1 = y_valid_prob[female_msk].mean() * (~female_msk).mean()

        ## compute theta
        objective = np.inf
        for theta in np.concatenate((-np.logspace(-2, 2, 10000), np.logspace(-2, 2, 10000))):
            m0 = (y_valid_prob[female_msk] * (y_valid_prob[female_msk] * (2.0 - theta / P_Y0S0) >= 1)).mean() / E_X0
            m1 = (y_valid_prob[~female_msk] * (y_valid_prob[~female_msk] * (2.0 + theta / P_Y0S1) >= 1)).mean() / E_X1
        ...
        ## apply theta
        nval = len(y_valid)
        y_valid_leveraged = np.zeros((nval,))
        for i in range(nval):
            if s_valid[i] == 0:
                y_valid_leveraged[i] = y_valid_prob[i] * (2.0 - thetahat / P_Y0S0) >= 1
            else:
                y_valid_leveraged[i] = y_valid_prob[i] * (2.0 + thetahat / P_Y0S1) >= 1

        compiled_scores_valid_list.append(y_valid_leveraged.reshape(-1,1))
    ...
    log.flush()
    return base, leverage, leverage_time, compiled_scores_list, compiled_scores_valid_list
