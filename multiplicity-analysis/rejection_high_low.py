...
Baseline vs Rejection (high and low fairness bin) and EqOdds for rejection
"""
...
for i in range(10):
    ...
    bin_scores_dic = {label: [(elem,meo,acc) for elem, meo, acc, label_elem in zip(score_rejection[i], meo_list, acc_list, bin_labels) if label_elem == label] for label in set(bin_labels)}
...
# =============================================================================
# originally reported acc/meo bucket means
# t = np.linspace(0,1,100)
...
ax.plot( v_plot_mean, t, label = "Rej High Fair(Acc:{:.3f}, EO violation:{:.2f})".format(Acc_level,EO_level))
...
EO_level = np.mean(bin_meo)
Acc_level = np.mean(bin_acc)
ax.plot( v_plot_mean, t, label = "Rej Low Fair(Acc:{:.3f}, EO violation:{:.2f})".format(Acc_level,EO_level))
ax.fill_betweenx(t, v_plot_mean+ v_plot_std, np.where(v_plot_mean - v_plot_std<0, 0, v_plot_mean - v_plot_std),  alpha=0.2)
...
EO_level = (x_bin_start+x_bin_end)/2
Acc_level = (y_bin_start+y_bin_end)/2
ax.plot( v_plot_mean, t, label = "Baseline (Acc:{:.3f}, EO violation:{:.2f})".format(Acc_level,EO_level))
...
EO_level = (x_bin_start+x_bin_end)/2
Acc_level = (y_bin_start+y_bin_end)/2
ax.plot( v_plot_mean, t, label = "Thresholded Baseline (Acc:{:.3f}, EO violation:{:.2f})".format(Acc_level,EO_level))
ax.fill_betweenx(t, v_plot_mean+ v_plot_std, v_plot_mean - v_plot_std,  alpha=0.2)
...
v_plot_list = [] 
for i in range(10):
    meo_list, acc_list = get_eo_acc(data, score_hardt[i],i)
    ...
    # Calculate the bin labels for each point
    bin_labels = (x_bin_indices - 1) * num_bins + y_bin_indices
    bin_scores_dic = {label: [(elem,meo,acc) for elem, meo, acc, label_elem in zip(score_hardt[i], meo_list, acc_list, bin_labels) if label_elem == label] for label in set(bin_labels)}
...
EO_level = np.mean(bin_meo)
Acc_level = np.mean(bin_acc)
ax.plot( v_plot_mean, t, label = "EqOdds (Acc:{:.3f}, EO violation:{:.2f})".format(Acc_level,EO_level))
ax.fill_betweenx(t, v_plot_mean+ v_plot_std, v_plot_mean - v_plot_std,  alpha=0.2)
...
plt.tight_layout()
#plt.show()
plt.savefig(title + model_base + 'rejection_eqodds.pdf', dpi=300)
